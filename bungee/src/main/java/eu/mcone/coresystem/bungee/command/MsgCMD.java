...
public class MsgCMD extends Command implements TabExecutor {
    ...
    public void execute(final CommandSender sender, final String[] args) {
        if (sender instanceof ProxiedPlayer) {
            ...
            if (args.length < 1) {
                BungeeCoreSystem.getInstance().getMessenger().sendSender(sender, "§4Bitte Benutze: §c/msg §c<Player | toggle> §c[<Nachricht>]");
                ...
                if (t != null) {
                    if (!p.equals(t)) {
                        if (!t.getFriendData().getBlocks().contains(p.getUuid()) || p.hasPermission("system.bungee.chat.private.bypass")) {
                            ...
                            BungeeCoreSystem.getInstance().getMessenger().sendSender(sender, "§c" + args[0] + "§4 hat dich blockiert!");
                        }
                    } else {
                        BungeeCoreSystem.getInstance().getMessenger().sendSender(sender, "§4Du kannst dich nicht selbst anschreiben, Dummkopf!");
                    }
                } else {
