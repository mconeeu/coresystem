...
public class NickManager implements eu.mcone.coresystem.api.bukkit.player.NickManager {
    ...
    public void setNicks(Player p) {
        for (CorePlayer cp : instance.getOnlineCorePlayers()) {
            if (cp.isNicked()) {
                ...
                connection.sendPacket(player != p ? new PacketPlayOutNamedEntitySpawn(((CraftPlayer) p).getHandle()) : new PacketPlayOutRespawn(ep.getWorldServer().worldProvider.getDimensionManager(), 0, ep.world.P(), ep.playerInteractManager.getGameMode()));
    ...
    private void setNick(Player p, String name, eu.mcone.coresystem.api.core.player.SkinInfo skin) {
        ...
        Bukkit.getScheduler().runTaskLater(CoreSystem.getInstance(), () -> {
            ...
            ep.playerConnection.sendPacket(new PacketPlayOutRespawn(ep.getWorldServer().worldProvider.getDimensionManager(), 0, ep.world.P(), ep.playerInteractManager.getGameMode()));
