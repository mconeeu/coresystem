...
class PartyMemberInventory extends CoreInventory {
    ...
    PartyMemberInventory(Player p, String member) {
        ...
        setItem(InventorySlot.ROW_3_SLOT_4, new ItemBuilder(Material.NETHER_STAR).displayName("§7Zum §ePartyleader§7 promoten").create(), e -> {
            BukkitCoreSystem.getInstance().getChannelHandler().createSetRequest(p, "CMD", "party promote " + member);
            p.closeInventory();
        });

        setItem(InventorySlot.ROW_3_SLOT_6, new ItemBuilder(Material.BARRIER).displayName("§4Aus der Party kicken").create(), e -> {
