...
class PlayerSettingsInventory extends CoreInventory {
    ...
    PlayerSettingsInventory(Player p) {
        ...
        setItem(InventorySlot.ROW_3_SLOT_3, cp.getSettings().isEnableFriendRequests() ?
                        new ItemBuilder(Material.LIME_DYE, 1).displayName("§a§lAktiviert").lore("§7§oKlicke zum akzeptieren von", "§7§oFreundschaftsanfragen").create() :
                        new ItemBuilder(Material.ROSE_RED, 1).displayName("§c§lDeaktiviert").lore("§7§oKlicke zum ablehnen von", "§7§oFreundschaftsanfragen").create(),
                e -> {
        ...
        if (p.hasPermission("system.bungee.nick")) {
            ...
            setItem(InventorySlot.ROW_3_SLOT_5, cp.getSettings().isAutoNick() ?
                            ...
                            new ItemBuilder(Material.ROSE_RED).displayName("§c§lDeaktiviert").lore("§7§oKlicke, um automatisch genickt", "§7§ozu werden").create(),
                    e -> {
        ...
        setItem(InventorySlot.ROW_4_SLOT_1, new ItemBuilder(Material.IRON_DOOR).displayName("§7§l↩ Zurück zum Profil").create(), e -> {
    ...
    private ItemBuilder getSenderItem(PlayerSettings.Sender sender) {
        switch (sender) {
            ...
            case NOBODY:
                return new ItemBuilder(Material.ROSE_RED).displayName("§c§lVon Niemandem");
            default:
