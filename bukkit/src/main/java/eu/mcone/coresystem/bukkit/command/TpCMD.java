            ...
            if (cmd.getName().equalsIgnoreCase("tp")) {
                if (args.length == 1) {
                    if (p.hasPermission("system.bukkit.tp")) {
                        ...
                        if (target == null) {
                            p.sendMessage(BukkitCoreSystem.config.getConfigValue("Prefix") + "§4Der Spieler §f" + args[0] + " §4konnte nicht gefunden werden!");
                            return true;
                        }
                        p.teleport(target.getLocation());
                        p.sendMessage(BukkitCoreSystem.config.getConfigValue("Prefix") + "§2Du wirst zu §f" + args[0] + "§2 teleportiert!");
                        return true;
                    } else {
                    ...
                    if (p.hasPermission("system.bukkit.tp.others")) {
                        ...
                        if (target1 == null) {
                            ...
                            p.sendMessage(BukkitCoreSystem.config.getConfigValue("Prefix") + "§4Der Spieler §f" + args[1] + " §4konnte nicht gefunden werden!");
                            return true;
                        }
                        target1.teleport(target2.getLocation());
                        p.sendMessage(BukkitCoreSystem.config.getConfigValue("Prefix") + "§2Der Spieler §f" + args[0] + "§2 wird zu §f" + args[1] + "§2 teleportiert!");
                        return true;
                    } else {
                        p.sendMessage(BukkitCoreSystem.config.getConfigValue("Prefix") + "§4Du hast keine Berechtigung für diesen Befehl!");
                    }
                    ...
                    if (p.hasPermission("system.bukkit.tp.pos")) {
                        ...
                        p.sendMessage(BukkitCoreSystem.config.getConfigValue("Prefix") + "§2Du wirst zu §f" + args[0] + " " + args[1] + " " + args[2] + "§2 teleportiert!");
                    } else {
                        p.sendMessage(BukkitCoreSystem.config.getConfigValue("Prefix") + "§4Du hast keine Berechtigung für diesen Befehl!");
                    }
                } else {
                    p.sendMessage(BukkitCoreSystem.config.getConfigValue("Prefix") + "§4Bitte benutze §c/tp [<Spieler>] <Zielspieler> §4oder §c/tp <x> <y> <z>");
                    return true;
                }
            }
        } else {
