...
class FriendInventory extends CoreInventory {
    ...
    FriendInventory(Player p, String friend) {
        ...
        setItem(InventorySlot.ROW_3_SLOT_3, new ItemBuilder(Material.ENDER_PEARL, 1).displayName("§7Teleportieren").create(), e -> {
            p.closeInventory();
            BukkitCoreSystem.getInstance().getChannelHandler().createSetRequest(p, "CMD", "jump " + friend);
        });
        setItem(InventorySlot.ROW_3_SLOT_5, new ItemBuilder(Material.CAKE, 1).displayName("§7In §5Party §7einladen").create(), e -> {
            p.closeInventory();
            BukkitCoreSystem.getInstance().getChannelHandler().createSetRequest(p, "CMD", "party invite " + friend);
        });
        setItem(InventorySlot.ROW_3_SLOT_7, new ItemBuilder(Material.BARRIER, 1).displayName("§4Freund entfernen").create(), e -> {
            p.closeInventory();
            BukkitCoreSystem.getInstance().getChannelHandler().createSetRequest(p, "CMD", "friend remove " + friend);
        });
        setItem(InventorySlot.ROW_4_SLOT_1, new ItemBuilder(Material.IRON_DOOR, 1).displayName("§7§l↩ Zurück zum Freundemenü").create(), e -> {
