    ...
    PartyInventory(Player p) {
        ...
        BukkitCoreSystem.getInstance().getChannelHandler().createGetRequest(p, member -> {
            if (!member.equals("false")) {
                ...
                setItem(InventorySlot.ROW_6_SLOT_1, new ItemBuilder(Material.IRON_DOOR, 1).displayName("§7§l↩ Zurück zum Profil").create(), e -> {
                    p.playSound(p.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASS, 1, 1);
                    new ProfileInventory(p);
                });

                if (isPartyLeader)
                    setItem(InventorySlot.ROW_6_SLOT_5, new ItemBuilder(Material.BARRIER, 1).displayName("§cParty löschen").create(), e -> {
                        p.closeInventory();
                        BukkitCoreSystem.getInstance().getChannelHandler().createSetRequest(p, "CMD", "party delete");
                    });

                setItem(InventorySlot.ROW_6_SLOT_9, new ItemBuilder(Material.SLIME_BALL, 1).displayName("§4Party verlassen").create(), e -> {
                    p.closeInventory();
                    BukkitCoreSystem.getInstance().getChannelHandler().createSetRequest(p, "CMD", "party leave");
                });
            } else {
                setItem(InventorySlot.ROW_3_SLOT_5, new ItemBuilder(Material.CAKE, 1).displayName("§5Party erstellen").create(), e -> {
                    p.closeInventory();
