...
class StatsCategoryInventory extends CoreInventory {
    ...
    StatsCategoryInventory(Player p, Stats stats) {
        ...
        setItem(InventorySlot.ROW_2_SLOT_5, new ItemBuilder(Material.ITEM_FRAME, 1).displayName("§3§l" + stats.getGamemode().getLabel() + " §8| §7Global Stats").lore(
                "§7Platz: §f" + result[0],
                "§7Kills: §f" + result[1],
                "§7Tode: §f" + result[2]
                ).create()
        ...
        setItem(InventorySlot.ROW_3_SLOT_1, new ItemBuilder(Material.IRON_DOOR, 1).displayName("§7§l↩ Zurück zum Stats Menü").create(), e -> {
